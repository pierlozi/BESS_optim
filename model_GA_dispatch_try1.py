...
def dispatcher_GA(design):
    ...
    m.DoD = Param(initialize = 75)
    m.cyclelife = Param(initialize = 2700)
    ...
    m.Pr_BES = Param(initialize = design.Pr_BES)
    m.Er_BES = Param(initialize = design.Er_BES)
...
xtry = [np.linspace(-10,10,21), np.linspace(-10,10,21)]
y1 = try_funct(xtry)[0]
y2 = try_funct(xtry)[1]
...
class ProblemWrapper(Problem):
    ...
    def _evaluate(self, xs, out, *args, **kwargs):
        res = []
        for x in xs:
            res.append(try_funct(x))
        
        out['F'] = np.array(res)

prob = ProblemWrapper(n_var=2, n_obj=2, xl=[-200.,-25.], xu = [200.,25.])

algo = NSGA2(pop_size=200)

stop_criterium = ('n_gen',100)
