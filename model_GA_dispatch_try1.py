...
def dispatcher_GA(design): #outputs the LCOS
    ...
    #with the BESS    
    m.P_ch = pyo.Var(m.iIDX, domain=NonNegativeReals)
    m.P_dch = pyo.Var(m.iIDX, domain = NonNegativeReals)
    m.P_RES = pyo.Var(m.iIDX, domain = NonNegativeReals)
    m.P_curt = pyo.Var(m.iIDX, domain = NonNegativeReals)
    
    m.Pr_BES = pyo.Param(initialize = design.Pr_BES)
    m.Er_BES = pyo.Param(initialize = design.Er_BES)
    
    m.SOC = pyo.Var(m.iIDX, domain=NonNegativeReals)
    m.SOC_ini = pyo.Var(domain=NonNegativeReals)
    ...
    m.bin_dch = pyo.Var(m.iIDX, domain=Binary)


    m.P_dg = pyo.Var(m.iIDX, domain = NonNegativeReals) #hourly power of diesel
    m.Pr_dg = pyo.Var(domain=NonNegativeReals) #power rating of diesel
    ...
    BES_cyclelife.append(round(value(m.cyclelife)/cycles_y[-1]))
...
prob = ProblemWrapper(n_var=2, n_obj=1, xl=[190.,15.], xu = [210.,20.])

algo = NSGA2(pop_size=10)

stop_criterium = ('n_gen',2)
