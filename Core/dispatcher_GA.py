...
def MyFun(design, bin_var, Er, Pr, DoD): #bin_var is to tell if power and energy rating are variables (bin_var=True) or input parameters (bin_var=False) for the dispatcher
    ...
    coefficients = np.polyfit(DoDs, cyclelifes, 6)

    cyclelife = int(np.polyval(coefficients, DoD))
    ...
    m.DoD = pyo.Param(initialize = DoD)
    m.cyclelife = pyo.Param(initialize = cyclelife)
    ...
    if bin_var:
        ...
        m.Pr_BES = pyo.Param(initialize = Pr) 
        m.Er_BES = pyo.Param(initialize = Er)
