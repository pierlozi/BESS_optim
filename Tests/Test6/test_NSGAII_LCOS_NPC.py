...
class ProblemWrapper(Problem):
    ...
    def _evaluate(self, designs, out, *args, **kwargs):
        ...
        for x in designs:
            ...
            LCOS, _ , NPC, _ = LCOS_funct.MyFun(design, \
                                    E_dch = sum(data_time['P_dch']),\
                                    res_val_bin = True
                                    )

            res.append([LCOS, NPC[0]])
        
        out['F'] = np.array(res)

#the variables are in order Er_BES, Pr_BES, DoD

problem = ProblemWrapper(n_var=3, n_obj=2, xl=[0.,0.,20.], xu = [850.,85.,80.], vtype=int)
...
df = pd.DataFrame(np.concatenate((X,F), axis = 1), columns = ['Er', 'Pr', 'DoD', 'LCOS','NPC'])
df.to_excel('test_NSGAII_LCOS_NPC.xlsx')


#%% Display 
coefficients = np.polyfit(df.LCOS.values, df.NPC.values, best_polyfit_degree.MyFun(df.LCOS.values, df.NPC.values ))
