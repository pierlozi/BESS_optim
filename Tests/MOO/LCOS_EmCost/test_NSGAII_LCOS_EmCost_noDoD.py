...
problem = ProblemWrapper(n_var=2, n_obj=2, xl=[0.,0.], xu = [E_lim,P_lim], vtype=int)
...
df = pd.DataFrame(np.concatenate((X,F), axis = 1), columns = ['Er', 'Pr', 'LCOS','EmCost'])
df.to_excel('test_NSGAII_LCOS_EmCost_noDoD.xlsx')
