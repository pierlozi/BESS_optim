...
problem = ProblemWrapper(n_var = 4, n_obj = 2, xl=[0.,0.,0., 60.], xu = [E_lim,P_lim, 40., 100.], vtype=int)
...
df = pd.DataFrame(np.concatenate((X,F), axis = 1), columns = ['Er', 'Pr', 'SOC_min', 'SOC_max', 'LCOS','EmCost'])
#df.to_excel('test_NSGAII_SOC_max_LCOS_EmCost.xlsx')
