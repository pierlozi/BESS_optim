...
from Core import dispatcher_dsctd, dispatcher_dsctd_copy
...
data_m, data_time_m = dispatcher_dsctd_copy.MyFun(design, True)
data_time_m['Datetime'] = pd.to_datetime(data_time_m['Datetime'], format = '%Y-%m-%d %H:%M:%S')
data_time_m.set_index('Datetime', inplace=True)
...
ax.plot(data_time_m['P_dg'][24*day_start_display:24*(day_start_display+days_display)], color=None, label='DG')
ax.plot(data_time_m['P_curt'][24*day_start_display:24*(day_start_display+days_display)],color=None, label='Curt')
ax.plot(data_time_m['P_prod'][24*day_start_display:24*(day_start_display+days_display)], color=None, label='Prod')
ax.plot(data_time_m['P_load'][24*day_start_display:24*(day_start_display+days_display)], color=None, label='Load')
ax.plot(data_time_m['P_BES'][24*day_start_display:24*(day_start_display+days_display)], color=None, label='BES')
...
ax_SOC = ax.twinx()
ax_SOC.set_ylabel("SOC [-]")
ax_SOC.plot(data_time_m['SOC'][24*day_start_display:24*(day_start_display+days_display)], color='purple')
