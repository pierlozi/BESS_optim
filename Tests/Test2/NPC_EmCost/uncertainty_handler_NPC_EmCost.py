...
df = pd.read_excel('test_NSGAII_NPC_EmCost_DoD_update.xlsx', index_col = 0, header= 0)
df_p = pd.read_excel('test_NSGAII_NPC_EmCost_plus_10_DoD_update.xlsx', index_col = 0, header= 0)
df_m = pd.read_excel('test_NSGAII_NPC_EmCost_minus_10_DoD_update.xlsx', index_col = 0, header= 0)
...
chart_f0 = alt.Chart(df, title = "Objective Space").mark_circle(color = 'blue').encode(
        alt.X('NPC').scale(zero=False).title('NPC [million €]'),
...
chart_des.save('uncertainty_NPC_EmCost_des.png')
chart_obj.save('uncertainty_NPC_EmCost_obj.png')
# %%
